...
**테스트는 세션 env 의 `QMD_RECALL_LOG` 를 물려받으면 안 된다** — 62개 테스트 파일 중 51개가 `{...process.env}` 를 그대로 자식에 넘기고 이 변수를 한 번도 주지 않으므로, 사용자가 그 env 를 전역으로 켜 두면 테스트가 **라이브 진단 로그에 쓴다**(실측: `hook-main.test.mjs` 1회 실행에 57줄, `~/.cache/qmd/recall-diag.jsonl` 에 traceback 610건 누적. 전부 첫 프레임이 `hook_main.py`). 파일별로 고치지 않고 **러너 진입점**에서 막는다 — `npm test` 는 `QMD_RECALL_LOG= node --test` 다(빈 문자열이면 `os.environ.get` 이 falsy 라 미설정과 같고, 테스트가 명시로 주는 경로는 그대로 먹는다). 직접 `node --test <파일>` 로 돌리면 이 가드가 없으므로, 예외를 일부러 던지는 파일은 자기 헬퍼에서도 비운다(`hook-main.test.mjs` 의 `cleanEnv`). 이건 `managerEnv` 가 포트 8483 을 assert 로 거부하는 것과 같은 격리 규칙이다 — 테스트가 제품 데이터를 건드리면 안 된다. **worker 부작용을 단정하는 테스트는 `QMD_LOCK_BASE`를 격리한다.** `update.sh`의 worker 는 전역 writer 락(`$TMPDIR/qmd-auto-context-locks-<user>` — `_QMD_UID`는 `id -un`이라 **사용자 이름**이다)을 잡지 못하면 `acquire_lock`이 즉시 실패해 `SKIP: qmd update already running`으로 **exit 0** 한다 — 기다리지 않는다. 다투는 상대는 같은 스위트의 다른 worker 와 **실제 세션의 SessionStart 훅**이므로, 격리하지 않으면 단독 실행은 늘 통과하고 부하 중에만 터진다(flake 로 보인다). **대기 예산(`QMD_TEST_WAIT_TIMEOUT_MS`)을 늘리는 것으로는 고쳐지지 않는다** — worker 가 포기하고 나가므로 아무리 기다려도 산출물이 생기지 않는다. 실측으로 손상 원장 격리 테스트가 이 형태였고, 전역 락을 살아 있는 pid 로 점유하면 100% 재현된다(수정 전 실패 → 격리 추가 후 같은 조건에서 통과). 짝이 되는 규칙: **worker 가 필요 없는 테스트는 `QMD_SKIP_BACKGROUND_WORKER`로 fork 자체를 끈다**(락 경합도 임시 디렉터리 잔해도 사라진다). 즉 worker 를 쓸 것이면 락을 격리하고, 쓰지 않을 것이면 fork 를 끈다 — 둘 중 하나는 반드시 한다. 테스트 작성 시 주의: `execFileSync`는 반드시 `encoding:'utf8'`을 줄 것 (없으면 Buffer 반환 → `.trim()` 에러). 병렬 실행에서 `core/__pycache__`가 spurious 실패를 유발할 수 있어 `.gitignore` 처리돼 있다.
...
- `wiki_compile_enqueue.py` / `wiki_compile_worker.py` — 자동 compile source는 `raw`/`session` Markdown만 허용하며, relative path의 어느 segment라도 `.`로 시작하면 skip/reject한다. `.agents/skills`, `.claude`, `.codex`, `.github`, `.auto-context` 같은 agent/tooling metadata를 wiki로 승격하지 않기 위한 기본 안전장치다.
...
- **빈 출력은 정상 동작일 수 있다.** 데몬 부재/timeout/결과 0건/sandbox/yield — 모두 의도적으로 무출력 종료한다. 빈 출력 ≠ 버그. **정상인지 버그인지 가르려면 `QMD_RECALL_LOG=<파일>`을 켜고 `qmd_recall_selection` 줄의 `reason`을 보라** (`event_disabled`/`no_keywords`/`no_collections`/`daemon_unreachable`/`query_failed`/`no_results_after_filter`/`selected`). **모든 줄에 `project_root` 귀속이 붙는다.** `QMD_RECALL_LOG`는 프로젝트별 파일이 아니라 **전역 파일 하나**라(env 하나가 한 경로를 준다) 귀속이 없으면 여러 프로젝트의 진단이 섞여 "이 프로젝트의 recall이 나아졌는가"를 물을 수 없다 — 3.8 백필 검증에서 실제로 막혀 census 델타로 우회했다. 값은 `find_project_config`의 `projectRoot`이고 **cwd가 아니다**(하위 디렉터리에서 시작한 세션이 한 프로젝트로 묶여야 한다. 단 그 불변성의 범위는 HOME 하위다 — `_project_search_dirs`가 HOME 밖에서는 부모를 탐색하지 않는다. 거기엔 설정도 없어 `no_collections`로 끝나므로 묶을 진단이 애초에 없다). **인자가 아니라 모듈 상태**(`set_log_project_root`)로 두는 이유는 기록 지점이 20곳이 넘고 절반이 조기 return이라 스레딩하면 다음에 추가되는 지점이 빠뜨리기 때문이다(`hook_main.py`가 per-site try/except를 한 곳으로 모은 것과 같은 클래스). writer는 `log_recall_event`와 `log_score_observation` **두 벌**이므로 한쪽만 고치면 조용히 갈린다. cwd를 알기 전 줄(`empty_stdin`·`invalid_payload_json`·`prompt_too_short`)은 키를 두고 값만 `null`이다 — 키 부재(귀속 도입 전 포맷)와 "아직 모른다"는 다른 사실이다. 이 로그는 파일에만 쓰고 stdout(모델 컨텍스트)엔 절대 안 나가며, env가 없으면 no-op다. index_enqueue도 게이팅으로 skip하는 경우(pending/optout/event 비활성/non-collection-path)는 정상 무출력이다.
