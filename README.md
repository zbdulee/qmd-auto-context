...
```
core/        recall.py · update.sh · posttool.py · index_enqueue.py · sync.py · config.py · keywords.py · resolve_paths.py · collection_match.py · agy_local_install.py
hooks/       run-hook (단일 디스패처) · hooks.json · hooks-codex.json
skills/      sync · query · update · hint (수동 워크플로우)
...
편집 후 자동 인덱싱: PostToolUse 훅이 편집 파일을 dirty 큐에 쌓으면, launchd worker(60초 주기)가 백그라운드에서 해당 컬렉션만 `qmd collection add`+`qmd update`+`qmd embed`해 자동으로 최신 상태를 유지한다(macOS 전용).
...
| `PostToolUse` posttool | `hint` | 특정 편집 파일 기준 연속성 힌트 |
| `PostToolUse` index | `sync` | 훅이 놓친 CUD 변경 감지 후 dirty queue enqueue |

### sync

`sync` skill은 `.auto-context.json`의 `collectionPaths`를 스캔해 이전 snapshot과 현재 파일 상태를 비교한다. 생성/수정/삭제가 감지된 collection만 dirty queue에 넣고, 실제 qmd 갱신은 기존 worker가 처리한다. 삭제 처리는 파일 단위 qmd 명령이 아니라 `qmd update`의 removed 처리에 맡긴다.
...
`query` skill은 훅의 `UserPromptSubmit` recall과 같은 `core/recall.py` 경로를 수동으로 실행한다. `.auto-context.json` opt-in, `QMD_QUERY_FIXTURE`, `QMD_DAEMON_URL`, minScore/topN 필터와 빈 출력 정책을 그대로 따른다.
...
`update` skill은 훅의 `SessionStart`와 같은 `core/update.sh` 경로를 수동으로 실행한다. `.auto-context.json` opt-in, risky path 검사, qmd 부재 시 graceful no-op 정책을 그대로 따른다.
...
`hint` skill은 훅의 `PostToolUse` posttool과 같은 `core/posttool.py` 경로를 수동으로 실행한다. 특정 파일 path와 파일 내용을 payload로 만들어 연속성 힌트를 요청한다.
