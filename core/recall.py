...
# 카드 파일 읽기 상한(문자). 849장 중 최대 파일이 2938자라 8192는 2.8배 여유다.
# 상한을 두는 이유: UserPromptSubmit은 blocking hook이고 이 읽기는 데몬 결과(limit 8)
# 건수만큼 반복된다 — 8×8KB=64KB로 유계. 사용자가 injectSummaryMaxChars를 크게 올리면
...
AUTO_BLOCK_START_RE = re.compile(r"<!--\s*qmd:auto:start[^>]*-->")
AUTO_BLOCK_END_RE = re.compile(r"<!--\s*qmd:auto:end\s*-->")
HTML_COMMENT_RE = re.compile(r"<!--.*?-->", re.DOTALL)
# 모든 자동 생성 카드 머리에 붙는 상투구. 정보가 0이라 주입에서 뺀다.
AUTO_DISCLAIMER_RE = re.compile(r"^>\s*Auto-generated by qmd-auto-context.*$", re.MULTILINE)
...
def extract_card_body(text: str, body_start: int) -> str:
    ...
    start = AUTO_BLOCK_START_RE.search(body)
    if start:
        end = AUTO_BLOCK_END_RE.search(body, start.end())
        if end:
            auto, manual = body[start.end():end.start()], body[end.end():]
        else:
            # 읽기 상한에 잘려 종료 마커를 못 본 경우: 시작 마커 뒤 전부를 auto로 본다.
            auto, manual = body[start.end():], ""
    else:
        # auto 마커가 없는 카드(수동 작성/구버전)는 본문 전체가 내용이다.
        auto, manual = body, ""
    auto = _strip_leading_summary_heading(
        HTML_COMMENT_RE.sub("", AUTO_DISCLAIMER_RE.sub("", auto))
    ).strip()
    manual = HTML_COMMENT_RE.sub("", AUTO_DISCLAIMER_RE.sub("", manual)).strip()
    joined = "\n\n".join(part for part in (auto, manual) if part)
    return re.sub(r"\n{3,}", "\n\n", joined).strip()


def truncate_summary(text: str, limit: int) -> str:
    """카드 본문을 limit 문자로 자른다. limit 0이면 본문 주입 자체를 끈다.

    문장 경계에서 자르는 이유: 절단면이 문장 중간이면 모델이 잘린 절을 완결된 사실로
    읽을 수 있다(한국어 서술문은 뒤에 부정·조건이 붙는다). 경계가 limit의 60% 앞에
    있으면(= 잘라 버리는 양이 과하면) 문자 단위로 자르고 절단 표시를 붙인다.
    """
    ...
    return head.rstrip() + SUMMARY_TRUNCATION_MARK
...
def read_wiki_meta(result: dict, config: dict, cwd: str, summary_max_chars: int = DEFAULT_INJECT_SUMMARY_MAX_CHARS) -> dict:
    ...
    """
    meta = {"status": "generated", "reviewed": False, "title": "", "summary": "", "displayPath": ""}
    path = resolve_wiki_result_path(result, config, cwd)
    if path is None:
        return meta
    try:
        text = path.read_text(encoding="utf-8")[:wiki_card_read_limit(summary_max_chars)]
    except OSError:
        return meta
    ...
    for line in text[3:end].splitlines():
        stripped = line.strip()
        for key in ("status", "reviewed", "createdBy", "title"):
...
def annotate_wiki_result(result: dict, config: dict, cwd: str, summary_max_chars: int) -> None:
    """wiki role 결과에 `_wiki_*` 메타를 붙인다(카드 파일 읽기 1회).

    본문·title·표시 경로는 wiki role 결과에만 붙으므로, raw 결과에는 어떤 경우에도
    본문이 실리지 않는다(wikiOnly 경계 유지).
    """
    meta = read_wiki_meta(result, config, cwd, summary_max_chars)
    ...
    summary = truncate_summary(meta.get("summary", ""), summary_max_chars)
    if summary:
...
def format_context(results: list[dict], prefix_style: str = "full", collection_roles: dict | None = None) -> str:
    ...
    for result in results:
        ...
        # 카드 본문을 2칸 들여 붙인다. 들여쓰기로 "이 항목에 속한 인용"임이 드러나고,
        # 본문 안 헤딩이 주입문의 최상위 섹션처럼 보이지 않는다.
    ...
    if has_summary:
        ...
        lines.append("위 들여쓴 문단은 해당 wiki 카드 본문이다(길면 절단). 요약으로 충분하면 파일을 열지 말고, 부족할 때만 위 경로를 Read.")
    lines.append("필요시 참조.")
    return "\n".join(lines)
...
def main():
    ...
    # 카드 본문 주입 예산. 0이면 본문 없이 경로+title만 넣는다(예전 포맷).
    summary_max_chars = qmd_config.coerce_nonneg_int(
        config.get("injectSummaryMaxChars", DEFAULT_INJECT_SUMMARY_MAX_CHARS),
        DEFAULT_INJECT_SUMMARY_MAX_CHARS,
    )
    ...
    for result in results:
        ...
        if roles.get(result.get("_collection", "")) == "wiki":
            annotate_wiki_result(result, config, cwd, summary_max_chars)
