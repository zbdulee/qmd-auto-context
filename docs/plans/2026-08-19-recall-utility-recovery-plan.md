...
| `wiki-ineligible` | **상태형 유지** | 일괄 재검증이 host CLI 호출 = 사용자 계정 과금. 지시할 무료 경로가 3.8 전까지 없다 |
...
- **남는 것**: `wiki-ineligible`의 지시형 전환은 3.8(무료로 증명 가능한 부분집합의 백필 도구)에
  의존한다. 도구가 없는 상태에서 지시를 붙이면 그 지시는 유료 재컴파일로 읽힌다.
