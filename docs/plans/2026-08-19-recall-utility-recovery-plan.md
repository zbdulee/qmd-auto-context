...
2026-08-21 dry-run 실측(`scripts/wiki-card-census.py`, 읽기 전용). 대상은 `verified` + `createdBy` +
`sourceRevisions` 없음이고 단일 파일 소스 + 매니페스트 컴파일 시각을 가진 카드다.

| 분류 | service-engineering | ai-proxy | rccar | ktlo-check | 가야 할 경로 |
|---|---|---|---|---|---|
| **건전 백필 가능** (원문 커밋 < 컴파일 시각, 클린) | **91** | **9** | **4** | **1** | 3.8 |
| 원문이 컴파일보다 새로움 | 436 | 14 | 9 | 1 | 재컴파일 (3.2) |
...
**두 시각은 반드시 파싱해서 비교한다.** git `%cI`는 로컬 오프셋(`+09:00`)으로, 매니페스트 `ts`는 UTC(`Z`)로
찍힌다. 문자열로 비교하면 같은 순간이 9시간 늦게 읽히고, 편향이 **한 방향**이라 "원문이 컴파일보다 새로움"이
과다 집계된다(= 건전 백필 가능이 과소 집계 — 이 한 줄이 44장과 105장을 갈랐다). 판정 함수는
`wiki-card-census.py:at`이고 백필 스크립트도 같은 규칙을 쓴다.

**그리고 이 오류는 재현성으로 잡히지 않는다.** 같은 비교를 세 번 실행하면 세 번 같은 답이 나온다 — 실제로 이
숫자는 세 번 "일치"로 확인됐고 세 번 틀렸다. 측정값을 신뢰하려면 재실행이 아니라 **다른 구현으로 같은 답이
나오는지**를 봐야 한다(지금 표는 census와 백필 스크립트 두 벌이 독립적으로 같은 값을 낸다).
...
- **기대 효과**: 네 프로젝트 합계 105장(91+9+4+1). 이 항목의 값은 그 숫자만이 아니라 **436장이 재컴파일
  대상이고 167장이 source-missing 대상이라는 분해**에도 있다 — 그것이 3.2와 3단계의 실제 작업량이다.
