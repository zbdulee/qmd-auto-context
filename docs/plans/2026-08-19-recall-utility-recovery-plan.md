...
| 층 | 문제 | 대표 수치 |
|---|---|---|
| ① 카드 자격 | 주입 자격을 갖춘 카드가 거의 없다 | service-engineering 1270장 중 fresh 174장(14%) — 실효 140장 |
...
service-engineering 실측(`verified` + `sourceRevisions` 없음 = 751장, 그중 단일 파일 소스 + 컴파일
시각 보유 695장):

| 분류 | 수 | 가야 할 경로 |
|---|---|---|
| **건전 백필 가능** (원문 커밋 < 컴파일 시각, 클린) | **40** | 3.8 |
| 원문이 컴파일보다 새로움 | 487 | 재컴파일 (3.2) |
| 원문 소실 | 167 | source-missing (3단계) |
| 워킹 트리 dirty | 1 | 보류 |
| 매니페스트 `ts` 없음 / 다중 소스 | 56 | 판정 불가 |
...
- **기대 효과**: service-engineering 40장. 크지 않다. 이 항목의 값은 **487장이 재컴파일 대상이고
