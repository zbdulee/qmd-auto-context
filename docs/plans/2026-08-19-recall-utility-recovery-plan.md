...
- **수정**: 숫자 보고 문구를 dedup 힌트와 같은 형태로 바꾼다 — "이것은 사용자 요청과 독립적인
  백그라운드 유지보수다. 지금 처리해라" + 실행할 절차. 자율 처리 경로는 이미 증명돼 있다.
- **주의**: `notice_once`는 echo하는 순간 marker를 쓴다. 즉 전달 여부와 무관하게 4시간 창을
  태운다. 지시형으로 바꿀 때 이 부분도 같이 본다.
- **검증**: 다음 SessionStart 이후 `source-missing.jsonl`의 대기 건수가 줄어드는지 본다.
...
### 3.8 건전하게 백필 가능한 카드만 되살린다 (범위가 작다)
...
2026-08-20 dry-run 실측(`scratchpad/census.py`, 읽기 전용). 대상은 `verified` + `createdBy` +
`sourceRevisions` 없음이고 단일 파일 소스 + 매니페스트 컴파일 시각을 가진 카드다.

| 분류 | service-engineering | ai-proxy | rccar | ktlo-check | 가야 할 경로 |
|---|---|---|---|---|---|
| **건전 백필 가능** (원문 커밋 < 컴파일 시각, 클린) | **40** | **4** | 0 | 0 | 3.8 |
| 원문이 컴파일보다 새로움 | 487 | 19 | 13 | 2 | 재컴파일 (3.2) |
| 원문 소실 | 167 | 0 | 1 | 0 | source-missing (3단계) |
| 판정 불가 (매니페스트 `ts` 없음 / 다중 소스) | 56 | 3 | 0 | 45 | — |
| 워킹 트리 dirty | 1 | 0 | 0 | 0 | 보류 |

service-engineering 수치는 8/19 측정과 정확히 일치한다(40/487/167/56/1) — 재현성 확인.
...
git으로 검증하면 40장이다 — 10배 차이이고, 나머지 375장에 찍으면 **낡은 카드에 신선 배지를 다는 것**이다.

- **수정**: 위 판정을 그대로 구현한 read-only 스크립트 1개(먼저 `--dry-run`으로 분류만 출력).
  git 이력이 없는 파일·다중 소스 카드는 건드리지 않는다.
- **기대 효과**: service-engineering 40장 + ai-proxy 4장. 크지 않다. 이 항목의 값은 **487장이 재컴파일 대상이고
  167장이 source-missing 대상이라는 분해**에 있다 — 그것이 3.2와 3단계의 실제 작업량이다.
- **검증**: 백필 후 그 40장이 recall에 나오는지, `dropped_stale`이 줄어드는지 본다.
...
| 4 | **3.8** 건전 백필 40장 | dry-run 먼저 | 범위가 작다. 값은 487/167 분해에 있다 |
...
| 8 | **3.3** 지시형 알림 | 3.5 이후 | 고아 목록을 함께 실어야 한다 |
