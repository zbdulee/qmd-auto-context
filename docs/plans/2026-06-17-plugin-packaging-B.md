...
- Produces: `cleanup_legacy_global_hooks(platform, config)` — 글로벌 설정에서 `adapters/<platform>/wrapper.py` 및 legacy qmd hook 제거(원자적, 비-qmd 보존). 봐야 할 경로(codex 2차 리뷰 확장):
  ...
  - inline `[hooks]`에 managed marker가 있으면 **TOML 파서로 제거**(경고만으론 중복 해소 안 됨).
...
- codex일 때 `${CODEX_HOME:-$HOME/.codex}/hooks.json` 정리 + 같은 layer `config.toml`의 inline `[hooks]`(managed marker 있으면 TOML 제거) + `${CODEX_HOME}/*.config.toml`(profile)·`$PWD/.codex/`(repo-local) 탐지 시 **경고 출력**(자동 편집 안 함).
...
- **codex 2차 plan 리뷰 반영:** Task 1 codex값=문서 확정/agy값=라이브 분리, Task 2 codex manifest에 git `source.url`+`policy`+`category`, Task 5에 CODEX_HOME/profile/repo-local 경로 + inline `[hooks]` TOML 제거 + malformed no-overwrite 테스트, Task 10을 marketplace 검증 뒤로. codex의 공식 필드명(`git-subdir`/`policy.*`)은 §배포 git 실측에서 재확인 단서로 남김.
