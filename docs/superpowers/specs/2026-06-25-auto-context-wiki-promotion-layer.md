...
- **governance**: 자동 compile은 초벌이다. lint가 의심 지점을 큐잉하고, 사람은 그 지점만 검토한다.
...
- 첫 단계에서 완전 자동 compile/lint를 구현하지 않는다.
...
- `compile.enabled`: 자동 compile 활성화 여부. 초기 구현은 false/read-only 중심.
...
2. wiki 결과가 충분하면 wiki 결과만 inject한다.
3. wiki 결과가 없거나 낮거나, raw 검증이 필요한 경우 `raw` collection을 backfill query한다.
4. 출력에는 source tier를 명시한다.

예시 출력:

```text
관련 문서:
- [wiki] .auto-context/wiki/decisions/config-layout.md - Config layout decision
...
초기 wiki는 Hermes `llm-wiki` 패턴을 프로젝트 로컬 coding context에 맞춘다.
...
초기 자동화는 wiki page를 바로 쓰지 않고 candidate queue를 만든다.
...
```json
{"ts":"2026-06-25T00:00:00Z","trigger":"user_approved_decision","title":"Config lives under .auto-context/settings.json","summary":"...","sources":["session"],"suggestedType":"decision","status":"pending"}
```

### Trigger

- `user_approved_decision`: 사용자가 "이 방향으로 하자", "확정" 등 승인.
- `repeated_recall`: 같은 raw 문서/주제가 여러 번 recall됨.
- `cross_file_conclusion`: 여러 파일을 분석해야 얻은 결론.
- `manual`: 사용자가 "wiki에 정리해" 요청.

초기 구현은 `manual`부터 시작한다.
...
### Phase 3 — manual compile candidate

- manual skill 또는 core command로 candidate 추가.
- 사용자가 명시 요청한 내용을 `compile/candidates.jsonl`에 append.
- 아직 wiki page 자동 생성은 하지 않음.
