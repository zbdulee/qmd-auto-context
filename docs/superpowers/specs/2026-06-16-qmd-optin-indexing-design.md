...
- `update.sh`(237행) embed 후 `launchctl kickstart -k`는 **SIGKILL**이라 데몬이 clean checkpoint 없이 죽는다 → WAL truncate가 보장되지 않을 수 있음. 본 설계로 대량 임베딩 트리거가 사라지면 실무 영향은 작지만, 별도 이슈로 남긴다.
