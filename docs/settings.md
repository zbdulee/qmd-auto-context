...
전이입니다 — 개명 되돌리기·`git checkout`·문서 재생성이 실측된 원인(개명)의 가장 흔한
...
**원자적**입니다 — 쓰기가 실패해도 원본이 잘리지 않아야 합니다. 이 모듈의 정책이 "카드가
유일한 기록일 수 있으므로 자동 삭제 금지"인데 카드를 만지는 유일한 쓰기 경로가 데이터를
파괴하면 정책과 구현이 반대 방향을 가리킵니다). 재지정 시 옛 `sourceHash` 같은 "옛 파일
