...
- [x] P5-3: agy — marketplace install은 "unknown marketplace"(import 기반)라 불가. **`--agy-local`로 잔상/무림 등록 완료** → `.agents/hooks.json` PostToolUse(matcher `write_to_file|replace_file_content`)에 posttool+index gemini. (실제 agy 편집 발동 검증은 사용자 agy 사용 시; 등록 구조 확인됨.)
