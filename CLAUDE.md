...
### 설정 해석 우선순위
`config.load_project_config`는 cwd에서 위로 올라가며 `.auto-context.json`을 찾고(없으면 레거시 `.agents/qmd-recall.json` fallback), 그마저 없으면 **cwd 폴더명을 단일 컬렉션으로** fallback. 컬렉션이 비면 빈 출력.
