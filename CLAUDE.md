...
**3층 구조: 플랫폼 무관 코어 1벌 + 얇은 hooks 디스패처 + plugin-managed 백엔드.**

```
core/      ← 모든 로직. backend_manager.sh + 플랫폼/도메인 무관 core. stdin {prompt,cwd} → stdout 훅 JSON
hooks/     ← run-hook 단일 디스패처 + hooks.json/hooks-codex.json. backend ensure/kick 후 코어로 패스스루
...
- `backend_manager.sh` — plugin runtime backend lifecycle SSOT. qmd version check(`>=2.5.3 <3.0.0`), daemon ensure/reload, warm, logrotate, async `index_worker.sh` kick, explicit legacy cleanup. Hooks must keep stdout silent; manual skills may print install guidance. qmd 자동 설치/업그레이드는 하지 않는다.
...
- `keepalive.sh` — one-shot vec warm ping. manager가 SessionStart/manual update에서 opportunistic 실행한다.
