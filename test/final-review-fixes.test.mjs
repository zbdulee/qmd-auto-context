...
test('MAJOR: install.sh 모든 config 쓰기가 원자적 (직접 open w 금지)', () => {
...
test('MAJOR: 깨진 settings.json → 기존 데이터 보존(빈 {}로 덮지 않음)', () => {
  ...
  try {
    ...
    try {
      execFileSync('bash', ['install.sh'], {
        env: { ...process.env, HOME: home, QMD_FAKE_PLATFORMS: 'claude', QMD_INSTALL_SKIP_BACKEND: '1', QMD_INSTALL_SKIP_SELFTEST: '1', QMD_MIGRATE_SCAN: join(home, 'none') },
      });
    } catch { threw = true; }
...
test('MAJOR: uninstall.sh backend 제거 계획 포함', () => {
  ...
  try {
    ...
    assert.match(out, /unload|launchctl|backend|\.config\/qmd/i, 'backend 제거 계획 없음');
  } finally {
    rmSync(home, { recursive: true, force: true });
  }
});
