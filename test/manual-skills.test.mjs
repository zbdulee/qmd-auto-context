...
test("manual skills expose enable-compile sync query update wiki-compile wiki-review wiki-dedup wiki-source-repair wiki-backfill only; hint remains hook-only", () => {
  ...
  assert.deepEqual(skillDirs, ["enable-compile", "query", "sync", "update", "wiki-backfill", "wiki-compile", "wiki-dedup", "wiki-review", "wiki-source-repair"]);
...
test("plugin descriptions list manual skills without hint", () => {
  ...
  for (const file of files) {
    ...
    assert.match(text, /sync\/query\/update\/wiki-compile\/wiki-review\/wiki-dedup\/wiki-source-repair\/wiki-backfill\/enable-compile manual skills/);
