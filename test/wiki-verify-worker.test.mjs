...
function setupProject({ verify = {}, extractorArgv = null, extractor = null, cardStatus = 'generated', cardExtraFm = '', jobOverrides = {}, withSource = true } = {}) {
  ...
  writeFileSync(join(dir, CARD_REL), cardText(cardStatus, cardExtraFm));
...
test('inconclusive delete: legacy reviewed scalar does not protect a generated card', () => {
  ...
  try {
    runVerifyWorker(project);
    assert.equal(existsSync(join(project, CARD_REL)), false, 'status-only lifecycle applies configured delete');
    const log = jsonl(join(project, '.auto-context', 'compile', 'verify-log.jsonl'));
    assert.equal(log.at(-1).result, 'deleted');
  } finally { removeTemp(project); }
});
